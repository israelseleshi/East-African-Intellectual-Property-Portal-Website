...
router.post('/submit', authenticateToken, async (req, res) => {
    try {
        ...
        applicantName, applicantNameAmharic, applicantType, nationality, email, telephone, addressStreet, addressZone, wereda, city, houseNo, state, zipCode, poBox, fax, 
        // Mark Info
        markName, markType, markDescription, colorIndication, priority, priorityCountry, 
        // Nice Classes
        niceClasses, 
        // Mark Image (Base64)
        markImage, 
        ...
        try {
            ...
            if (existingClientId) {
                ...
                const [clientRows] = await connection.execute('SELECT id FROM clients WHERE id = ?', [existingClientId]);
                ...
                await connection.execute(`INSERT INTO clients (id, name, local_name, type, nationality, email, address_street, city, zip_code, address_zone, wereda, house_no, po_box, telephone, fax, created_at, updated_at) 
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`, [
                    clientId,
                    applicantName,
            ...
            await connection.execute(`INSERT INTO trademark_cases (
            ...
            if (niceClasses && Array.isArray(niceClasses)) {
                ...
                const eipaFormRecord = eipaFormData;
            ...
            if (pdfBase64) {
                ...
                await connection.execute(`INSERT INTO mark_assets (id, case_id, type, file_path, created_at) 
                     VALUES (?, ?, 'POA', ?, NOW())`, [
                    crypto.randomUUID(),
                    caseId,
            ...
            if (markImage && markImage.startsWith('data:image')) {
                ...
                await connection.execute(`UPDATE trademark_cases SET mark_image = ? WHERE id = ?`, [`/forms-download/${imageFilename}`, caseId]);
            }
            // 5. Add to case history
            await connection.execute(`INSERT INTO case_history (id, case_id, user_id, action, old_data, new_data, created_at)
                 VALUES (?, ?, ?, ?, ?, ?, NOW())`, [
                crypto.randomUUID(),
                caseId,
                userId,
            ...
            await connection.execute(`INSERT INTO deadlines (id, case_id, type, due_date, is_completed, created_at)
