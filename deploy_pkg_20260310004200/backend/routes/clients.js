...
router.get('/', authenticateToken, async (req, res) => {
    try {
        ...
        const [rows] = await pool.execute(`SELECT * ${sql} ORDER BY name ASC LIMIT ? OFFSET ?`, [...params, limit, offset]);
        res.json({
            data: rows,
            meta: {
                total,
...
router.post('/', authenticateToken, async (req, res) => {
    try {
        ...
        await query('INSERT INTO clients (id, name, local_name, type, nationality, residence_country, email, address_street, address_zone, wereda, city, house_no, zip_code, po_box, telephone, fax) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [
            clientId,
            data.name,
...
router.get('/:id', authenticateToken, async (req, res) => {
    try {
        ...
        const [trademarks] = await pool.execute('SELECT id, mark_name, status, jurisdiction, filing_number, created_at FROM trademark_cases WHERE client_id = ? ORDER BY created_at DESC', [parsed.data.id]);
...
router.patch('/:id', authenticateToken, async (req, res) => {
    try {
        ...
        if (data.localName !== undefined || data.local_name !== undefined)
            updates.local_name = data.localName || data.local_name || null;
        if (data.type !== undefined)
            updates.type = data.type;
        if (data.nationality !== undefined)
            updates.nationality = data.nationality;
        if (data.residence_country !== undefined || data.residenceCountry !== undefined)
            updates.residence_country = data.residence_country || data.residenceCountry || null;
        if (data.email !== undefined)
            updates.email = data.email;
        if (data.addressStreet !== undefined || data.address_street !== undefined)
            updates.address_street = data.addressStreet || data.address_street || null;
        if (data.addressZone !== undefined || data.address_zone !== undefined)
            updates.address_zone = data.addressZone || data.address_zone || null;
        if (data.wereda !== undefined)
            updates.wereda = data.wereda;
        if (data.city !== undefined)
            updates.city = data.city;
        if (data.houseNo !== undefined || data.house_no !== undefined)
            updates.house_no = data.houseNo || data.house_no || null;
        if (data.zipCode !== undefined || data.zip_code !== undefined)
            updates.zip_code = data.zipCode || data.zip_code || null;
        if (data.poBox !== undefined || data.po_box !== undefined)
            updates.po_box = data.poBox || data.po_box || null;
        if (data.telephone !== undefined || data.phone !== undefined)
            updates.telephone = data.telephone || data.phone || null;
        if (data.fax !== undefined)
            updates.fax = data.fax;
