...
router.get('/case/:caseId', authenticateToken, async (req, res) => {
    try {
        ...
        const [rows] = await pool.execute(`SELECT cn.*, u.full_name as user_name
       FROM case_notes cn
       LEFT JOIN users u ON cn.user_id = u.id
       WHERE cn.case_id = ? AND cn.deleted_at IS NULL
       ORDER BY cn.is_pinned DESC, cn.created_at DESC`, [parsed.data.caseId]);
        res.json(rows);
    }
    catch (error) {
...
router.get('/:id', authenticateToken, async (req, res) => {
    try {
        ...
        const [noteRows] = await pool.execute(`SELECT cn.*, u.full_name as user_name
       FROM case_notes cn
       LEFT JOIN users u ON cn.user_id = u.id
       WHERE cn.id = ? AND cn.deleted_at IS NULL`, [parsed.data.id]);
        ...
        const [replyRows] = await pool.execute(`SELECT cn.*, u.full_name as user_name
       FROM case_notes cn
       LEFT JOIN users u ON cn.user_id = u.id
       WHERE cn.parent_note_id = ? AND cn.deleted_at IS NULL
       ORDER BY cn.created_at ASC`, [parsed.data.id]);
...
router.post('/', authenticateToken, async (req, res) => {
    try {
        ...
        await pool.execute(`INSERT INTO case_notes (id, case_id, user_id, note_type, content, is_private, parent_note_id)
       VALUES (?, ?, ?, ?, ?, ?, ?)`, [
            id,
            parsed.data.caseId,
            req.user?.id ?? null,
...
router.patch('/:id', authenticateToken, async (req, res) => {
    try {
        ...
        await pool.execute(`UPDATE case_notes SET ${fields.join(', ')}, updated_at = NOW() WHERE id = ? AND deleted_at IS NULL`, values);
...
router.post('/:id/reply', authenticateToken, async (req, res) => {
    try {
        ...
        const caseId = (parentRows[0]).case_id;
        const replyId = crypto.randomUUID();
        await pool.execute(`INSERT INTO case_notes (id, case_id, user_id, content, parent_note_id, note_type)
       VALUES (?, ?, ?, ?, ?, 'REPLY')`, [replyId, caseId, req.user?.id ?? null, parsedBody.data.content, parsedParams.data.id]);
        res.status(201).json({
            id: replyId,
            caseId,
