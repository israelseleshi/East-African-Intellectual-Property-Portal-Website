...
router.post('/', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (!clientId) {
            ...
            await connection.execute('INSERT INTO clients (id, name, type, nationality, email, address_street, city, zip_code) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [
                clientId,
                data.applicantName,
        ...
        await connection.execute('INSERT INTO trademark_cases (id, jurisdiction, mark_name, mark_type, status, flow_stage, client_id, user_id, color_indication, priority, client_instructions, remark, mark_image, mark_description) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [
            newCaseId,
        ...
        if (data.niceClasses && data.niceClasses.length > 0) {
            for (const niceClass of data.niceClasses) {
                ...
                await connection.execute('INSERT INTO nice_class_mappings (case_id, class_no, description) VALUES (?, ?, ?)', [newCaseId, classNo, description]);
            }
        }
