...
router.get('/case/:caseId', authenticateToken, async (req, res) => {
    try {
        ...
        const [rows] = await pool.execute(`SELECT o.*, tc.mark_name
       FROM oppositions o
       JOIN trademark_cases tc ON o.case_id = tc.id
       WHERE o.case_id = ? AND o.deleted_at IS NULL
       ORDER BY o.created_at DESC`, [parsed.data.caseId]);
        res.json(rows);
    }
    catch (error) {
...
router.get('/:id', authenticateToken, async (req, res) => {
    try {
        ...
        const [rows] = await pool.execute(`SELECT o.*, tc.mark_name, c.name as client_name, tc.jurisdiction
       FROM oppositions o
       JOIN trademark_cases tc ON o.case_id = tc.id
       JOIN clients c ON tc.client_id = c.id
       WHERE o.id = ? AND o.deleted_at IS NULL`, [parsed.data.id]);
        if (rows.length === 0) {
...
router.post('/', authenticateToken, async (req, res) => {
    try {
        ...
        if (!calculatedDeadline && parsed.data.oppositionDate) {
            const [jurisdictionRows] = await pool.execute(`SELECT j.opposition_period_days
         ...
         WHERE tc.id = ?`, [parsed.data.caseId]);
            if (jurisdictionRows.length > 0) {
                const days = (jurisdictionRows[0]).opposition_period_days || 60;
        ...
        await pool.execute(`INSERT INTO oppositions (id, case_id, opponent_name, opponent_address,
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
        ...
        if (calculatedDeadline) {
            await pool.execute(`INSERT INTO deadlines (id, case_id, due_date, type, description)
         VALUES (?, ?, ?, 'OPPOSITION_RESPONSE', ?)`, [crypto.randomUUID(), parsed.data.caseId, calculatedDeadline, `Response deadline for opposition by ${parsed.data.opponentName}`]);
        }
        res.status(201).json({
            id,
...
router.patch('/:id', authenticateToken, async (req, res) => {
    try {
        ...
        await pool.execute(`UPDATE oppositions SET ${fields}, updated_at = NOW() WHERE id = ? AND deleted_at IS NULL`, values);
    ...
    try {
        ...
        try {
            ...
            if (parsedBody.data.status === 'RESOLVED') {
                ...
                const caseId = oppRows[0]?.case_id;
                if (caseId) {
...
router.get('/dashboard/pending', authenticateToken, async (req, res) => {
    try {
        ...
        const [rows] = await pool.execute(`SELECT o.*, tc.mark_name, c.name as client_name,
       ...
       ORDER BY o.deadline_date ASC`, [days]);
        res.json(rows);
    }
    catch (error) {
