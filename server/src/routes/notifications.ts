...
router.get('/unread/count', authenticateToken, async (req: any, res) => {
    try {
        ...
        res.json({ unread_count: (rows as Array<{ unread_count: number }>)[0].unread_count });
    } catch (error) {
    ...
    try {
        ...
        const [rows] = await pool.execute(
            `SELECT n.*, tc.mark_name, c.name as client_name
             ...
             LEFT JOIN trademark_cases tc ON n.case_id = tc.id
             LEFT JOIN clients c ON tc.client_id = c.id
             WHERE n.id = ? AND n.recipient_id = ? AND n.deleted_at IS NULL`,
            [id, userId]
        );
...
router.patch('/read-all', authenticateToken, async (req: any, res) => {
    try {
        ...
        res.json({ success: true, marked_read: (result as { affectedRows: number }).affectedRows });
    } catch (error) {
...
router.post('/', authenticateToken, async (req: any, res) => {
    try {
        ...
        await pool.execute(
            `INSERT INTO notifications (id, recipient_type, recipient_id, case_id, type, 
             channel, subject, content, metadata)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [id, recipientType, recipientId, caseId || null, type, channel, 
