...
describe('Trademark Intake - 2. Kenya', () => {
  beforeEach(() => {
    cy.visit('/login');
  ...
  it('should submit Kenya intake with image upload', () => {
    // Applicant
    cy.get('input[placeholder="Enter full legal name in English"]').type('Serengeti Safari Group');
    cy.get('input[placeholder="ሙሉ ስም እዚህ ያስገቡ"]').type('Serengeti Safari Group');

    // Address
    cy.get('input[placeholder="Enter street address"]').type('Mombasa Road, Gateway Mall');
    cy.get('input[placeholder="Enter city name"]').type('Nairobi');
    cy.get('input[placeholder="Enter ZIP code"]').type('00100');

    // Contact
    cy.get('input[placeholder="+251 ..."]').type('+254712345678');
    cy.get('input[placeholder="name@example.com"]').type('bookings@serengetisafari.ke');
    cy.get('input[placeholder="Enter nationality"]').type('Kenyan');
    cy.get('input[placeholder="Enter residence country"]').type('Kenya');
    ...
    cy.get('textarea[placeholder="Describe the visual and literal elements of the mark..."]')
      .type('A circular logo with a silhouette of a giraffe and an acacia tree.');

    // Nice Classification (Class 39)
    cy.get('#nice-classification').scrollIntoView();
    cy.get('#nice-classification').find('div[class*="cursor-text"]').first().click();
    cy.get('input[placeholder="Search classes..."]').should('be.visible').type('39');
    cy.contains('div', 'Class 39').click({force: true});
    cy.get('body').click('topRight');
    cy.wait(500);
    
    // Goods & Services
    cy.get('textarea[placeholder="List specific goods and services for the selected classes..."]')
      .type('Transport; packaging and storage of goods; travel arrangement.', {force: true});
    ...
    cy.get('textarea[placeholder="List goods/services covered by priority claim"]').type('Transport and travel arrangement services');
    cy.get('span').contains('Documents accompany form').parent().click({force: true});

    // Signature
    cy.get('input[placeholder="Type name for digital signature"]').type('Safari Manager');
