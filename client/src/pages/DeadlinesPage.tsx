...
const JurisdictionFlag = ({ code, className = "h-4 w-6" }: { code: string, className?: string }) => (
  <span className={className}>{JURISDICTION_FLAGS[code] || '🌍'}</span>
);
                  ...
                  {filteredDeadlines.map((d) => {
                    ...
                    return (
                      <div key={d.id} onClick={() => navigate(`/deadlines/${d.id}`)} className="group flex items-center gap-4 px-6 py-4 hover:bg-muted/50 transition-colors cursor-pointer">
                        <div className={`shrink-0 flex flex-col items-center justify-center w-12 h-12 rounded-lg border ${isOverdue ? 'bg-red-500/10 border-red-200 text-red-600' : isUrgent ? 'bg-orange-500/10 border-orange-200 text-orange-600' : 'bg-primary/10 border-primary/20 text-primary'}`}>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            <Badge className={`text-xs ${DEADLINE_TYPE_COLORS[d.type?.toUpperCase() || 'GENERIC']}`}>{DEADLINE_TYPE_LABELS[d.type?.toUpperCase() || 'GENERIC'] || d.type}</Badge>
                          </div>
            ...
            <div className="p-6">
                  ...
                  return (
                    <div 
                      ...
                      className={`py-3 rounded-lg transition-all cursor-pointer relative text-center 
                        ...
                        ${isSelected(day) ? 'bg-primary text-white font-bold shadow-md scale-105 z-10' : hasDeadline ? 'bg-orange-500 text-white font-bold hover:bg-orange-600' : 'hover:bg-muted'}`}
                    >
                      {day}
              ...
              {selectedDate && (
                <div className="mt-6 pt-4 border-t animate-in fade-in slide-in-from-top-2">
                  ...
                  <div className="space-y-2">
                    {(() => {
                      const selectedDeadlines = filteredDeadlines.filter(d => d.due_date && selectedDate && d.due_date.startsWith(`${selectedDate.getFullYear()}-${String(selectedDate.getMonth() + 1).padStart(2, '0')}-${String(selectedDate.getDate()).padStart(2, '0')}`))
                        ...
                        <div key={d.id} className="p-3 bg-muted/50 rounded-lg flex items-center justify-between border border-border/50 hover:bg-muted transition-colors cursor-pointer" onClick={() => navigate(`/deadlines/${d.id}`)}>
                          <div className="flex-1 min-w-0 pr-4">
                            ...
                            <div className="text-xs text-muted-foreground line-clamp-2 leading-relaxed">
                              {d.client} • {(d as any).description || `Statutory deadline for ${DEADLINE_TYPE_LABELS[d.type?.toUpperCase() || 'GENERIC'] || d.type}.`}
                            </div>
                          </div>
